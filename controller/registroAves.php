...
if (
    ...
    try {
        ...
        $fechaHoraFormateadaIn = date('Y-m-d H:i:s');
        if ($fechaVa == null) {
            //llamo el modelo
            ...
            $sql1 =  "INSERT INTO `aves` (`idAves`, `especie`, `peso`, `estadoSalud`, `fechaIngreso`, `LoteAves_idLoteAves`) VALUES (NULL, :especie, :peso, :estadoSalud, :fechaHoraFormateadaIn,  :loteave);";
            ...
            $replace = str_replace("T", " ", $fechaHoraFormateada);
            if ($replace < $fechaHoraFormateadaIn) {
                header("Location: ../vista/pages/produccion/registroAves.php");
                $_SESSION['mensajeErr4'] = "La fecha de vacunacion es menor a la fecha de ingreso";
                ...
                $pdo = $conexion->conectar();
                //hago la consulta para insertar en la base de datos
                $sql1 =  "INSERT INTO `aves` (`idAves`, `especie`, `peso`, `estadoSalud`, `fechaVacunacion`, `fechaIngreso`, `LoteAves_idLoteAves`) VALUES (NULL, :especie, :peso, :estadoSalud, :fechaVa, :fechaHoraFormateadaIn,  :loteave);";
                $stmt1 = $pdo->prepare($sql1);
                $stmt1->bindParam(':especie', $especie, PDO::PARAM_STR);
                $stmt1->bindParam(':peso', $peso, PDO::PARAM_STR);
                $stmt1->bindParam(':estadoSalud', $estadoSalud, PDO::PARAM_STR);
